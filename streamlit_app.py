...
if not st.session_state.prefs_loaded:
    ...
    if _sb_url and _sb_key:
        try:
            ...
            pass   # falha silenciosa — app funciona sem preferências
        ...
        with st.status("Analisando orçamentos…", expanded=True) as status_box:
            ...
            for supplier_name, pdf_bytes in uploaded_files.items():
                try:
                    ...
                    n_done += 1
                    st.write(f"✅ **{supplier_name}** — {len(items)} item(s) extraído(s)")
            ...
            if supplier_items:
                ...
                try:
                    ...
                    status_box.update(
                        label=f"✅ {len(normalized)} itens prontos para revisão",
    ...
    if not final_items:
        ...
        with st.spinner("Gerando planilha…"):
            try:
                ...
                # generate_excel retorna (bytes, warning_ou_None)
                ...
                # Indica quantas preferências estão ativas
