...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if run_extraction:
        ...
        for n, (supplier_name, pdf_bytes) in enumerate(uploaded_files.items(), 1):
            status.markdown(f"<div style='font-size:0.88rem;color:#636366;'>Processando <b>{supplier_name}</b>…</div>", unsafe_allow_html=True)
            try:
    ...
    if not items:
        ...
        with col_fwd:
            if st.button("Gerar Excel →", type="primary"):
                st.session_state.final_items = df_to_items(edited_df, active_suppliers)
