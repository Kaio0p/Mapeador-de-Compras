...
# ── Apple Liquid Glass CSS — suporte a claro e escuro ─────────────────────────
st.markdown("""
<style>
...
h1 {
    ...
    letter-spacing: -0.03em !important; color: #1C1C1E !important;
    margin-bottom: 0.2rem !important; line-height: 1.15 !important;
}
h2 {
    font-size: 1.35rem !important; font-weight: 600 !important;
    letter-spacing: -0.02em !important; color: #1C1C1E !important;
}
h3 {
...
[data-testid="stSidebar"] h3 {
    ...
    color: #8E8E93 !important; margin: 1.4rem 0 0.5rem !important;
}
[data-testid="stSidebar"] hr {
    border: none !important; border-top: 1px solid rgba(0,0,0,0.07) !important; margin: 1rem 0 !important;
}
...
.stTextInput input, .stDateInput input, .stNumberInput input, .stTextArea textarea {
    ...
    font-size: 0.88rem !important; color: #1C1C1E !important;
...
.stDownloadButton > button:hover {
    transform: translateY(-1px) !important; box-shadow: 0 6px 22px rgba(40,167,69,0.45) !important;
}
...
[data-testid="stFileUploader"]:hover {
    border-color: rgba(0,113,227,0.55) !important; background: rgba(0,113,227,0.03) !important;
}
[data-testid="stFileUploaderDropzoneInstructions"] {
    font-size: 0.85rem !important; color: #636366 !important;
}

/* Alerts */
.stAlert { border-radius: 12px !important; border: none !important; backdrop-filter: blur(12px) !important; }
[data-testid="stAlert"][kind="info"], div[data-testid="stAlert"] {
    background: rgba(0,113,227,0.07) !important; border-left: 3px solid #0071E3 !important;
...
.stRadio label {
    ...
    border-radius: 8px !important; padding: 6px 14px !important; font-size: 0.85rem !important;
    transition: all 0.15s !important;
}

/* Progress */
.stProgress > div > div {
    background: linear-gradient(90deg, #0071E3, #34C759) !important; border-radius: 999px !important;
}
.stProgress > div {
    background: rgba(0,0,0,0.06) !important; border-radius: 999px !important; height: 4px !important;
}
...
/* ════════════════════════════════════════
   MODO ESCURO — sobrepõe seletores acima
   ════════════════════════════════════════ */
...
@media (prefers-color-scheme: dark) {
    ...
    h2 { color: #F5F5F7 !important; }
    h3 { color: #E5E5EA !important; }
    p, label, .stMarkdown, .stCaption { color: #AEAEB2 !important; }
    .stCaption, small { color: #636366 !important; }

    .stTextInput input, .stDateInput input, .stNumberInput input, .stTextArea textarea {
        background: rgba(44,44,46,0.9) !important; border: 1px solid rgba(255,255,255,0.1) !important;
        color: #F5F5F7 !important; box-shadow: none !important;
    }
    .stTextInput input:focus, .stTextArea textarea:focus {
        border-color: #0A84FF !important; box-shadow: 0 0 0 3px rgba(10,132,255,0.2) !important;
    }

    .stButton > button {
        background: rgba(44,44,46,0.9) !important; color: #0A84FF !important;
        border: 1px solid rgba(10,132,255,0.3) !important;
    }
    .stButton > button:hover {
        background: rgba(10,132,255,0.12) !important; border-color: rgba(10,132,255,0.6) !important;
    }
    .stButton > button[kind="primary"],
    button[data-testid="baseButton-primary"] {
    ...
    .stDownloadButton > button {
        background: linear-gradient(180deg, #30D158 0%, #25A244 100%) !important;
    }

    [data-testid="stFileUploader"] {
        background: rgba(44,44,46,0.6) !important;
        border: 1.5px dashed rgba(10,132,255,0.3) !important;
    }
    [data-testid="stFileUploader"]:hover {
        border-color: rgba(10,132,255,0.6) !important; background: rgba(10,132,255,0.06) !important;
    }
    [data-testid="stFileUploaderDropzoneInstructions"] { color: #8E8E93 !important; }

    div[data-testid="stAlert"] {
        background: rgba(10,132,255,0.1) !important; border-left: 3px solid #0A84FF !important;
    }

    .streamlit-expanderHeader {
        background: rgba(44,44,46,0.75) !important; border: 1px solid rgba(255,255,255,0.07) !important;
        color: #E5E5EA !important;
    }
    .streamlit-expanderHeader:hover { background: rgba(58,58,60,0.95) !important; }
    .streamlit-expanderContent {
        background: rgba(44,44,46,0.5) !important; border: 1px solid rgba(255,255,255,0.06) !important;
    }

    .stRadio label {
        background: rgba(44,44,46,0.8) !important; border: 1px solid rgba(255,255,255,0.1) !important;
        color: #E5E5EA !important;
    }

    [data-baseweb="select"] > div {
        background: rgba(44,44,46,0.85) !important; border-color: rgba(255,255,255,0.1) !important;
    }
    ...
    .glass-card .supplier-label { color: #636366 !important; }
    .glass-card .supplier-name  { color: #F5F5F7 !important; }
    .step-track {
        background: rgba(44,44,46,0.75) !important; border: 1px solid rgba(255,255,255,0.07) !important;
    }
    .step-dot.idle { background: rgba(255,255,255,0.1) !important; }
    .step-text .num { color: #E5E5EA !important; }
    .step-text .sub { color: #636366 !important; }
    .page-header .subtitle { color: #8E8E93 !important; }
    .section-title { color: #F5F5F7 !important; }
    .apple-divider { background: linear-gradient(90deg, transparent, rgba(255,255,255,0.08), transparent) !important; }
    .ref-label { color: #636366 !important; }
}

/* Streamlit injeta [data-theme="dark"] no body — cobrimos os dois seletores */
...
[data-theme="dark"] h1 { color: #F5F5F7 !important; }
[data-theme="dark"] h2 { color: #F5F5F7 !important; }
[data-theme="dark"] h3 { color: #E5E5EA !important; }
[data-theme="dark"] p,
[data-theme="dark"] label,
[data-theme="dark"] .stMarkdown { color: #AEAEB2 !important; }
[data-theme="dark"] .stCaption,
[data-theme="dark"] small { color: #636366 !important; }
...
/* ════════════════════════════════════════
   COMPONENTES CUSTOMIZADOS (ambos os modos)
   ════════════════════════════════════════ */
...
.glass-card .supplier-label {
    ...
    text-transform: uppercase; color: #8E8E93; margin-bottom: 4px;
}
.glass-card .supplier-name {
    font-size: 1.05rem; font-weight: 600; color: #1C1C1E; letter-spacing: -0.02em;
}
...
.alert-pill {
    ...
    color: #B25000; border-radius: 980px; padding: 4px 12px;
...
.suspect-card .suspect-title {
    font-weight: 600; color: #B25000; font-size: 0.84rem; margin-bottom: 4px;
}
.suspect-card .suspect-reason {
...
# ── Lê API key do Secrets e configura Gemini ─────────────────────────────────
def _get_sb_creds():
    try:
...
def _init_gemini():
    """Configura Gemini a partir dos Secrets — sem exibir a chave para o usuário."""
...
# Tenta configurar imediatamente ao carregar
_init_gemini()
...
with st.sidebar:
    ...
    # Indicador de status da API (sem campo de entrada)
    if st.session_state.api_key_ok:
        st.markdown(
    ...
    if run_extraction:
        ...
        with st.status("Analisando orçamentos…", expanded=True) as status_box:
            for supplier_name, finfo in uploaded_files.items():
                ...
                try:
                    if is_image:
                        # Arquivo de imagem direta (PNG ou JPEG)
                        ...
                        if is_img_pdf:
                            st.write(f"🔍 PDF escaneado detectado — enviando para OCR via Gemini Vision…")
        ...
        if suspect_items:
            with st.expander(f"⚠️ {len(suspect_items)} alerta(s) de validação — clique para ver", expanded=True):
                for it in suspect_items:
                    reasons_html = "".join(
                        f'<div>• {r}</div>' for r in (it.get("alert_reason") or [])
                    )
                    st.markdown(f"""
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "UND":   st.column_config.SelectboxColumn(
                    "UND", options=ALLOWED_UNITS, width="small",
