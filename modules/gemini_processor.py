...
SDK: google-genai (novo SDK unificado)
  - Cada tentativa cria um Client fresco via get_gemini_client() (round-robin)
  - Chamada: client.models.generate_content(model=..., contents=...)
  - Resposta: response.text

Anti-Rate-Limit:
  - Pool de chaves round-robin: cada tentativa usa a PROXIMA chave (get_gemini_client)
  - 1a falha 429: tenta próxima chave com delay mínimo (3s) — resolve se só 1 chave esgotou
  - 2a+ falhas: backoff exponencial (×1.5) com cap de 65s
...
from modules.llm_manager import get_gemini_client
...
def configure(api_key: str) -> None:
    """Retrocompatibilidade: no-op no novo SDK (stateless por design)."""
    pass
...
# ── Helpers para construir contents no formato do novo SDK ─────────────────────

def _build_image_part(b64_data: str, mime_type: str) -> types.Part:
    """
    Converte uma string base64 em um types.Part com bytes brutos
    para o novo SDK google-genai.
    """
...
def _call_vision_with_retry(
    ...
    """
    Chama o Gemini via novo SDK (google-genai) com retry e rotação
    round-robin de chave. A cada tentativa, cria um Client fresco
    via get_gemini_client().

    Estratégia anti-429 em 2 fases:
      FASE 1 — "Sprint de chaves" (tentativas 1..N_KEYS):
        Tenta TODAS as chaves disponíveis com delay mínimo de 3s entre cada.
        O objetivo é encontrar uma chave que não esteja throttled.

      FASE 2 — "Backoff" (tentativas N_KEYS+1..max_attempts):
        Todas as chaves foram tentadas e falharam. Agora aplica backoff com
        o delay explícito da API (ou base_delay), esperando a quota resetar.
    """
    from modules.llm_manager import _load_gemini_keys
    n_keys  = len(_load_gemini_keys()) or 1
    attempt = 0
    delay   = base_delay
    ...
    while attempt < max_attempts:
        try:
            ...
            # Detecta erros de rate limit / resource exhausted / 429
            is_rate_limit = any(kw in err_str for kw in [
                "429", "resource exhausted", "resourceexhausted",
                "rate limit", "quota", "too many requests",
            ])

            # Detecta erros de serviço indisponível / 503
            is_unavailable = any(kw in err_str for kw in [
            ...
            if is_rate_limit:
                ...
                if attempt <= n_keys:
                    # FASE 1: Sprint — tenta próxima chave com delay mínimo
                    wait = 3.0
                    logger.warning(
                        "[Gemini] 429 (tentativa %d/%d, sprint chave %d/%d). "
                        "Tentando proxima chave em %.0fs...",
                        attempt, max_attempts, attempt, n_keys, wait,
                    )
                else:
                    # FASE 2: Backoff — todas as chaves falharam, espera quota resetar
                    wait = explicit_delay or delay
                    logger.warning(
                        "[Gemini] 429 (tentativa %d/%d, backoff). "
                        "Todas as chaves esgotadas. Aguardando %.0fs...",
                ...
                attempt += 1
                logger.warning(
                    "[Gemini] Servico indisponivel (tentativa %d/%d). Aguardando %.0fs...",
                ...
                # Erro não recuperável — propaga imediatamente
                raise
...
def extract_items_from_images(
    ...
    if text_fallback and not images_b64:
        ...
        # Imagens PNG — monta contents com Parts de bytes
        contents = [vision_prompt]
        for b64 in images_b64:
            contents.append(_build_image_part(b64, "image/png"))

    raw   = _call_vision_with_retry(contents)
...
def extract_items_from_jpeg_images(
    ...
    # Imagens JPEG — monta contents com Parts de bytes
    contents = [vision_prompt]
    for b64 in images_b64:
        contents.append(_build_image_part(b64, "image/jpeg"))
...
def audit_purchase_map(
    ...
    try:
        ...
        # Monta contents — se temos imagens originais, inclui para auditoria visual
            ...
            for supplier_name, img_list in original_images.items():
                if isinstance(img_list, list):
                    for img_b64 in img_list[:3]:  # máx 3 páginas por fornecedor
                        contents.append(_build_image_part(img_b64, "image/png"))
