...
def _post_process_normalized(items: list) -> list:
    """Pós-processamento para itens normalizados (Camada 2 + 3)."""
    for item in items:
        if "unidade" in item:
            item["unidade"] = _normalize_unit(item.get("unidade", "UN"))
        marca = item.get("marca")
...
def normalize_and_match(
    ...
    NOTA IMPORTANTE sobre o response_schema:
    O passo de normalização é um prompt complexo (cruzamento de N fornecedores,
    agrupamento semântico, preservação de preços). Usar response_schema aqui
    faz o Gemini "travar" e gerar respostas absurdas (ex: 1 item chamado "BMW"
    sem preços) para satisfazer o schema. Por isso, response_schema é DESATIVADO
    neste passo — apenas response_mime_type="application/json" é usado.
    A validação de tipos é feita pelo Pydantic (Camada 2) logo após o parse.
    """
    time.sleep(_INTER_CALL_DELAY)
    ...
    # NÃO passa response_schema aqui — apenas força JSON puro.
    # O schema estrito causa alucinações no prompt complexo de normalização.
    ...
    # ── Guardrail: resultado plausível? ──────────────────────────────────────
    # Se o Gemini devolveu muito menos itens que o esperado (< 30% do input),
    # algo correu muito mal — provavelmente alucinação ou JSON truncado.
    if isinstance(data, list) and len(data) < max(1, total_input_items * 0.30):
        logger.warning(
            ...
            f"vs {total_input_items} itens de entrada. Tentando nova chamada sem schema..."
        )
        # Segunda tentativa com temperatura ainda mais controlada (via prompt reforçado)
