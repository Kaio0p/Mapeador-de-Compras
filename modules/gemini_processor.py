...
"""
gemini_processor.py
Extração e normalização via Gemini Flash.
...
# Modelo único — apontando sempre para a versão mais recente e capaz do flash
...
def _call_with_retry(parts, max_attempts: int = 8, base_delay: float = 20.0) -> str:
    ...
    while attempt < max_attempts:
        try:
            ...
            logger.warning(f"[{_MODEL_NAME}] 429 na tentativa {attempt}/{max_attempts}. Aguardando {wait:.0f}s...")
            if attempt >= max_attempts:
                raise RuntimeError(
                    f"Limite de requisições do Gemini atingido após {max_attempts} tentativas.\n\n"
                    f"O free tier do {_MODEL_NAME} tem limites estritos de requisições/minuto. "
