...
# Schema para normalização/cruzamento (array de objetos com sub-estrutura)
...
def normalize_and_match(
    ...
    prompt = NORMALIZATION_PROMPT.format(
        ...
        preferences=preferences_context or "Nenhuma preferências registrada ainda.",
    )

    raw = _call_with_retry(prompt, response_schema=_NORMALIZATION_RESPONSE_SCHEMA)
    try:
        data = json.loads(raw)
    except (json.JSONDecodeError, TypeError):
