...
def load_from_supabase(supabase_url: str, supabase_key: str) -> dict:
    ...
    try:
        ...
        logging.error(f"[Supabase] HTTP {e.code} ao carregar preferências: {e.read().decode()}")
    except urllib.error.URLError as e:
        logging.error(f"[Supabase] Falha de conexão ao carregar preferências: {e.reason}")
    except Exception as e:
        logging.error(f"[Supabase] Erro inesperado ao carregar preferências: {e}")
...
def load_catalog_from_supabase(supabase_url: str, supabase_key: str) -> list:
    ...
    Retorna lista de dicts com keys: nome_oficial, unidade_padrao.
    ...
    try:
        ...
        logging.error(f"[Supabase] HTTP {e.code} ao carregar catálogo: {e.read().decode()}")
    except urllib.error.URLError as e:
        logging.error(f"[Supabase] Falha de conexão ao carregar catálogo: {e.reason}")
    except Exception as e:
        logging.error(f"[Supabase] Erro inesperado ao carregar catálogo: {e}")
    return []
