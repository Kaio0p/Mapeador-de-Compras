...
def extract_text_from_pdf(pdf_bytes: bytes) -> tuple:
    ...
    Usa context manager para garantir liberação de memória em qualquer cenário.
    """
    try:
    ...
    try:
        ...
        doc.close()   # garante fechamento mesmo em caso de exceção
...
def extract_images_from_pdf(pdf_bytes: bytes, max_pages: int = 10) -> list:
    ...
    Usa try/finally para garantir liberação de memória em qualquer cenário.
    """
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    images_b64 = []
    try:
        for i, page in enumerate(doc):
            if i >= max_pages:
                break
            mat = fitz.Matrix(2.0, 2.0)   # DPI alto para melhor OCR
